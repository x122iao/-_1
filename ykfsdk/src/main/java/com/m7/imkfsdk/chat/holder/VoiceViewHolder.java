...
//    public CCPAnimImageView voiceLoading;
    ...
    public BaseHolder initBaseHolder(View baseView, boolean receive) {
        ...
        voicePlayAnim = baseView.findViewById(R.id.chatting_voice_play_anim_tv);
...
//        voicePlayFrameLayout = ((FrameLayout) baseView.findViewById(R.id.chatto_content_layout));
        voiceAnim = baseView.findViewById(R.id.chatting_voice_anim);
        voiceAnim.restBackground();
        voiceUnread = baseView.findViewById(R.id.chatting_unread_flag);
        voiceSecondView = baseView.findViewById(R.id.chatting_voice_second_tv);

        ll_voiceTotext = baseView.findViewById(R.id.ll_voiceTotext);
        ll_text_ok = baseView.findViewById(R.id.ll_text_ok);
        tv_vototx = baseView.findViewById(R.id.tv_vototx);//语音转文本 text
        rl_pb = baseView.findViewById(R.id.rl_pb);//语音转文本 布局
        pb_voice_text = baseView.findViewById(R.id.pb_voice_text);
        if (receive) {
            type = 5;
...
//            int duration = (detail.recordTime).intValue();
            ...
            if (adapterForce.mVoicePosition == position) {
                ...
                holder.voiceAnim.setBackgroundResource(R.drawable.ykfsdk_ykf_send_msg_bg);
                holder.voiceAnim.setWidth(DensityUtil.dp2px(getTimeWidth(duration)));
                holder.contentTv.setTextColor(activity.getResources().getColor(R.color.ykfsdk_voice_tv));
...
//                holder.contentTv.setText(Math.round(detail.recordTime) + "\"");
                ...
                holder.voicePlayAnim.setWidth(DensityUtil.dp2px(getTimeWidth(duration)));
                return;
            } else {
...
//                holder.contentTv.setText(Math.round(detail.recordTime) + "\"");
            ...
            holder.voiceAnim.setBackgroundResource(R.drawable.ykfsdk_ykf_send_msg_bg);
            holder.voicePlayAnim.setBackgroundResource(R.drawable.ykfsdk_ykf_send_msg_bg);
...
//            holder.contentTv.setText(detail.voiceSecond + "\"");
            ...
            if (adapterForce.mVoicePosition == position) {
                holder.voiceAnim.setVisibility(View.VISIBLE);
                holder.voiceAnim.startVoiceAnimation();
                holder.voiceAnim.setBackgroundResource(R.drawable.ykfsdk_ykf_receive_msg_bg);
                holder.voiceAnim.setWidth(DensityUtil.dp2px(getTimeWidth(duration)));
                holder.contentTv.setTextColor(activity.getResources().getColor(R.color.ykfsdk_voice_tv));
                holder.contentTv.setShadowLayer(2.0F, 1.2F, 1.2F, Color.parseColor("#ffffffff"));
                holder.contentTv.setVisibility(View.VISIBLE);
//                holder.contentTv.setText(detail.voiceSecond + "\"");
            ...
            holder.voiceAnim.setBackgroundResource(R.drawable.ykfsdk_ykf_receive_msg_bg);
            holder.voicePlayAnim.setBackgroundResource(R.drawable.ykfsdk_ykf_receive_msg_bg);
            holder.contentTv.setBackgroundColor(0);
        }
        ...
        if (!detail.withDrawStatus) {
            ...
            params.width = DensityUtil.dp2px(getTimeWidth(1)) - DensityUtil.dp2px(4) - measuredWidthTicketNum;
...
//        }
    ...
    private static void showPopWindows(View view, final VoiceViewHolder holder, final FromToMessage detail, final ChatActivity activity) {
        ...
        dataList.add(activity.getResources().getString(R.string.ykfsdk_voice_to_text_btn));
