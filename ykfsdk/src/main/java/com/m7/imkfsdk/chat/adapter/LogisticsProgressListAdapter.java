...
public class LogisticsProgressListAdapter extends RecyclerView.Adapter<LogisticsProgressHolder> {
     ...
     */
    private final boolean isFormMoreProgress;
    ...
    public LogisticsProgressHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View view = inflater.inflate(R.layout.ykfsdk_item_logistics_progress, parent, false);
    ...
    public void onBindViewHolder(@NonNull final LogisticsProgressHolder holder, final int position) {
        if (data != null && data.size() > 0) {
            if (getItemViewType(position) == TYPE_TOP) {
                ...
                //数据个数是1时，显示绿色，否则显示蓝色
                holder.view_dot.setBackgroundResource(data.size() == 1 ? R.drawable.ykfsdk_timelline_dot_bottom : R.drawable.ykfsdk_timelline_dot_first);
            } else if (getItemViewType(position) == TYPE_NORMAL) {
                holder.view_topline.setVisibility(View.VISIBLE);
                holder.view_bottomline.setVisibility(View.VISIBLE);
                holder.view_dot.setBackgroundResource(R.drawable.ykfsdk_timelline_dot_normal);
            } else {
                if (isFormMoreProgress) {
                    holder.view_bottomline.setVisibility(View.INVISIBLE);
                    holder.view_dot.setBackgroundResource(R.drawable.ykfsdk_timelline_dot_bottom);
                } else {
                    //最后一行，下面的竖线不显示
                    if (data.size() > 3) {
                        holder.view_bottomline.setVisibility(View.VISIBLE);
