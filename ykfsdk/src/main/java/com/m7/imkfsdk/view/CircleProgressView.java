...
public class CircleProgressView extends View {
    ...
    private final Path mClipArcPath = new Path();  // 要裁剪掉的扇形部分 B
    private final Path mClipBgPath = new Path();   // 整个View的背景 A，绘制部分为: A-B
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ykfsdk_CircleProgress);
        mProgress = typedArray.getInt(R.styleable.ykfsdk_CircleProgress_ykfsdk_circleProgress, 0);
        mCorner = typedArray.getDimensionPixelOffset(R.styleable.ykfsdk_CircleProgress_ykfsdk_circleCorner, 0);
        mStartAngle = typedArray.getInt(R.styleable.ykfsdk_CircleProgress_ykfsdk_startAngle, 315);
        mBackgroundColor = typedArray.getColor(R.styleable.ykfsdk_CircleProgress_ykfsdk_backgroundColor,
