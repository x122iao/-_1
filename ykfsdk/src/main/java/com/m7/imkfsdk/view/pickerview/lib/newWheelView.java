...
public class newWheelView extends View {
    ...
    public newWheelView(Context context, AttributeSet attrs) {
       ...
       /* textColorOut = getResources().getColor(R.color.pickerview_wheelview_textcolor_out);
        ...
        textSize = getResources().getDimensionPixelSize(R.dimen.ykfsdk_pickerview_textsize);//默认大小
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ykfsdk_pickerview, 0, 0);
            mGravity = a.getInt(R.styleable.ykfsdk_pickerview_ykfsdk_pickerview_gravity, Gravity.CENTER);
            textColorOut = a.getColor(R.styleable.ykfsdk_pickerview_ykfsdk_pickerview_textColorOut, textColorOut);
            textColorCenter = a.getColor(R.styleable.ykfsdk_pickerview_ykfsdk_pickerview_textColorCenter, textColorCenter);
            dividerColor = a.getColor(R.styleable.ykfsdk_pickerview_ykfsdk_pickerview_dividerColor, dividerColor);
            textSize = a.getDimensionPixelOffset(R.styleable.ykfsdk_pickerview_ykfsdk_pickerview_textSize, textSize);
            lineSpacingMultiplier = a.getFloat(R.styleable.ykfsdk_pickerview_ykfsdk_pickerview_lineSpacingMultiplier, lineSpacingMultiplier);
            a.recycle();//回收内存
        }
    ...
    private void initLoopView(Context context) {
        ...
        handler = new newMessageHandler(this);
        gestureDetector = new GestureDetector(context, new newLoopViewGestureListener(this));
    ...
    void smoothScroll(ACTION action) {//平滑滚动的实现
        ...
        mFuture = mExecutor.scheduleWithFixedDelay(new newSmoothScrollTimerTask(this, mOffset), 0, 10, TimeUnit.MILLISECONDS);
    }

    protected final void scrollBy(float velocityY) {//滚动惯性的实现
        cancelFuture();
        mFuture = mExecutor.scheduleWithFixedDelay(new newInertiaTimerTask(this, velocityY), 0, VELOCITYFLING, TimeUnit.MILLISECONDS);
    }
