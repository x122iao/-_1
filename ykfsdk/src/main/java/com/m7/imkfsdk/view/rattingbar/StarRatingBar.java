...
public class StarRatingBar extends View {
    ...
    public StarRatingBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ykfsdk_StarRatingBar);
        mDefaultStar = typedArray.getDrawable(R.styleable.ykfsdk_StarRatingBar_ykfsdk_defaultStar);
        mStar = typedArray.getDrawable(R.styleable.ykfsdk_StarRatingBar_ykfsdk_star);
        mDefaultStarColor = typedArray.getColor(R.styleable.ykfsdk_StarRatingBar_ykfsdk_defaultStarColor, Color.parseColor("#eeeeee"));
        mStarColor = typedArray.getColor(R.styleable.ykfsdk_StarRatingBar_ykfsdk_starColor, Color.parseColor("#ff9100"));
        mStarNum = typedArray.getInteger(R.styleable.ykfsdk_StarRatingBar_ykfsdk_starNum, 3);
        mStarStep = typedArray.getFloat(R.styleable.ykfsdk_StarRatingBar_ykfsdk_starStep, 1.0f);
        mStarGap = typedArray.getDimensionPixelOffset(R.styleable.ykfsdk_StarRatingBar_ykfsdk_starGap, 10);
        mStarSize = typedArray.getDimensionPixelOffset(R.styleable.ykfsdk_StarRatingBar_ykfsdk_starSize, 80);
        mRating = typedArray.getFloat(R.styleable.ykfsdk_StarRatingBar_ykfsdk_rating, 0f);
        mIsIndicator = typedArray.getBoolean(R.styleable.ykfsdk_StarRatingBar_ykfsdk_isIndicator, true);
        typedArray.recycle();
...
//        mRating = Math.round(mRating);
        mRating = (float) Math.ceil(mRating);
//        MoorLogUtils.i("StarRatingBar-选中", mRating);

        if(starRatingListener!=null){
            starRatingListener.onSelectStar(mRating);
        }
