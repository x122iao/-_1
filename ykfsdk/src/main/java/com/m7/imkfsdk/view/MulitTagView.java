...
public class MulitTagView extends LinearLayout {
    ...
    private final List<Option> optionLists = new ArrayList<>();
    ...
    public MulitTagView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
...
//        layoutManager.setFlexDirection(FlexDirection.ROW_REVERSE);
    ...
    class TagSelectAdapter extends RecyclerView.Adapter<TagSelectAdapter.MineContactViewHolder> {
        ...
        private final Set<Option> broadcas = new LinkedHashSet<>();
        ...
        public MineContactViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new MineContactViewHolder(layoutInflater.inflate(R.layout.ykfsdk_kf_xbotform_mulititem, parent, false));
        }
        ...
        public void onBindViewHolder(MineContactViewHolder holder, final int position) {
            ...
            if (bean.isSelected) {
                broadcas.add(bean);
                holder.mulit_checkbox.setChecked(true);
                holder.mulit_checkbox.setTextColor(FIleResourceUtil.getCurrentColor(context, R.attr.ykfsdk_ykf_theme_color_default));
                holder.rl_checkbg.setBackground(ContextCompat.getDrawable(mContext, R.drawable.ykfsdk_kf_bg_xbot_tiempress));
            } else {
                holder.rl_checkbg.setBackground(ContextCompat.getDrawable(mContext, R.drawable.ykfsdk_kf_bg_xbot_tiem));
                holder.mulit_checkbox.setChecked(false);
                holder.mulit_checkbox.setTextColor(context.getResources().getColor(R.color.ykfsdk_color_333333));
            }

            holder.mulit_checkbox.setText(bean.name);
            holder.mulit_checkbox.setOnClickListener(new OnClickListener() {
                @Override
        ...
        public class MineContactViewHolder extends RecyclerView.ViewHolder {
            ...
            CheckBox mulit_checkbox;
            RelativeLayout rl_checkbg;

            public MineContactViewHolder(View inflate) {
                super(inflate);
                mulit_checkbox = inflate.findViewById(R.id.mulit_checkbox);
                rl_checkbg=inflate.findViewById(R.id.rl_checkbg);
            }
        }
    }
    ...
    class TagSingleSelectAdapter extends RecyclerView.Adapter<TagSingleSelectAdapter.SingleTagViewHolder> {
        ...
        public SingleTagViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new SingleTagViewHolder(layoutInflater.inflate(R.layout.ykfsdk_kf_textview_flowlayout, parent, false));
        }
        ...
        void setTagView(SingleTagViewHolder holder, int position, Option option) {
            ...
            if (option.isSelected) {
                broadcas.clear();
                broadcas.add(option);
                holder.tv_title.setBackground(ContextCompat.getDrawable(mContext, R.drawable.ykfsdk_kf_bg_my_label_selected));
                int currentColor = FIleResourceUtil.getCurrentColor(mContext, R.attr.ykfsdk_ykf_theme_color_default);
                holder.tv_title.setTextColor(currentColor);
            } else {
                holder.tv_title.setBackground(ContextCompat.getDrawable(mContext, R.drawable.ykfsdk_kf_bg_my_label_unselected));
                holder.tv_title.setTextColor(ContextCompat.getColor(mContext, R.color.ykfsdk_kf_tag_unselect));
            }
        }
