 ...
 * @date: 2024/12/16
 */
class SeniorAdapter(private val itemList: List<Item>) : RecyclerView.Adapter<SeniorAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        if (position == selectedPosition) {
            holder.cons.setBackgroundResource(R.drawable.selected_background) // 选中时的背景
            holder.text2.setTextColor(Color.parseColor("#7C40D7"));
            holder.text1.setTextColor(Color.parseColor("#7C40D7"));


        } else {
            holder.cons.setBackgroundResource(R.drawable.unselected_background) // 未选中时的背景
        }
