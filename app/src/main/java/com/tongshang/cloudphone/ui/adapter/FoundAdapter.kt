 ...
 * @date: 2024/12/16
 */
class FoundAdapter(private val itemList: List<Item>) : RecyclerView.Adapter<FoundAdapter.MyViewHolder>() {
    ...
    inner class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val image: ImageView = itemView.findViewById(R.id.imageView44) // 假设 itemLayout 是整个 item 的布局
        val text: TextView = itemView.findViewById(R.id.textView) // 假设 itemLayout 是整个 item 的布局
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        if (position == selectedPosition) {
            holder.text1.setTextColor(Color.parseColor("#7186FF"));
            holder.text2.setTextColor(Color.parseColor("#7385FF"));
            holder.cons.setBackgroundResource(R.drawable.selected_background_found) // 选中时的背景
        } else {
            holder.cons.setBackgroundResource(R.drawable.unselected_background_found) // 未选中时的背景
        }
